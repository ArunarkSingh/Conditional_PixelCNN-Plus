...
def train_or_test(model, data_loader, optimizer, loss_op, device, args, epoch, mode = 'training'):
    ...
    for batch_idx, item in enumerate(tqdm(data_loader)):
        ...
        [my_bidict[name] if name in my_bidict else 0 for name in label_names],
        device=device, dtype=torch.float32
    )
        num_classes = len(my_bidict)

        cond_values = numeric_labels / (num_classes - 1) if num_classes > 1 else numeric_labels
        
        # Build a condition tensor. Assume model_input has shape [batch, channels, H, W]
        cond_tensor = cond_values.view(-1, 1, 1, 1).expand(model_input.size(0), 1,
                                                            model_input.size(2), model_input.size(3))

        model_output = model(model_input, sample=False, cond=cond_tensor)
